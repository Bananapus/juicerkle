...
func (tree *Tree) root() []byte {
	...
	i := 0
	for ; i < treeDepth; i++ {
		// Iterate through the tree until we find a non-zero (odd) index at depth i
		if index&(1<<i) != 0 {
	...
	// If we were able to iterate through the entire tree, the tree is full of zeroes and the root is z32
	if i == treeDepth {
		current = z32
	}

	// If i is greater than 30, we've already hashed the entire tree
	if i > 30 {
		return current
	}

	// For the rest of the tree, hash the current node with its sibling until we reach the root
	for ; i < treeDepth-1; i++ {
		if index&(1<<(i+1)) == 0 {
